...
### Wiring the CLI into `package.json`

Yes, these go in the existing `package.json` at the project root — the one `npm init -y` created back in Step 1. You're adding two keys to what's already there, not creating a separate file.

Open `package.json`. It should already have `name`, `version`, `main`, `type`, `dependencies`, etc. Add a `"bin"` field and a `"scripts"` field (or merge into the existing `"scripts"` block if `npm init` already created one with a placeholder `"test"` entry). Here's what a representative result looks like — the lines marked `// ← add` are the ones you're inserting, the rest is context you should already have:
...
- **`"scripts"`** gives you shorthand `npm run` commands. `npm run cli` invokes `tsx src/cli/index.ts`, so during development you run TypeScript source directly without a build step. The `--` separator before your CLI arguments (`npm run cli -- list --limit 5`) is an npm convention that forwards everything after it to the underlying script rather than having npm try to parse it.

- **`"bin"`** tells npm that when this package is installed (locally or globally), it should create an executable called `bsky-archive` that points at the compiled JavaScript in `dist/cli/index.js`. This doesn't do anything until you run `npm link` (or install the package), which is why we cover it in two stages below.

### During development — use the `cli` script
...
### For a system-wide command — build and link

Once you're happy with the CLI and want `bsky-archive` available as a standalone command from any directory:

```bash
npm run build          # compiles TypeScript → dist/
npm link               # symlinks the bin entry into your global node_modules
```

After this, you can run `bsky-archive list` from anywhere — not just from inside the project directory. The `#!/usr/bin/env node` shebang at the top of `src/cli/index.ts` is what makes the compiled `.js` file executable as a standalone script rather than requiring you to prefix it with `node`.

If you later want to remove the global symlink: `npm unlink -g bluesky-archive`.
