...
If this fails with `Invalid identifier or password`, work through these in order — it's almost always one of the first three:

1. **You're using your account password, not an App Password.** Bluesky rejects your real login password for API access. Generate one at bsky.app → Settings → Privacy and Security → App Passwords, and use that string as `BSKY_APP_PASSWORD`. It looks like `xxxx-xxxx-xxxx-xxxx`.
2. **`BSKY_IDENTIFIER` isn't a full handle.** It needs to be the whole handle, e.g. `yourname.bsky.social` (or your custom domain handle if you have one) — not just `yourname`, and not your email unless email login is explicitly enabled on the account.
3. **Stray quotes or whitespace in `.env`.** `dotenv` does *not* trim surrounding quotes the way a shell would. Don't wrap values in quotes — write `BSKY_APP_PASSWORD=xxxx-xxxx-xxxx-xxxx`, not `BSKY_APP_PASSWORD="xxxx-xxxx-xxxx-xxxx"`. Also watch for a trailing space if you copy-pasted the app password from the Bluesky UI.
4. **`.env` isn't where `dotenv` expects it.** By default `dotenv/config` loads `.env` from the current working directory, not the script's location. Confirm you're running `tsx` from the project root (where `.env` lives), not from inside `src/`.
5. **The app password was revoked or already used to create another session that got invalidated.** Delete it and generate a fresh one — this happens more often than it should.
6. **Confirm the values actually loaded**, rather than assuming — add a temporary line before `agent.login()`:
   ```typescript
   ...
   ```
   If either logs as `undefined`, it's a `.env` loading problem (points 3–4), not a credentials problem.
